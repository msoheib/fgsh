-- Make manual round advancement idempotent against duplicate controller clicks.
--
-- After a successful advance, games.current_round points at the next round
-- before that round row is created. A duplicate RPC call in that gap used to
-- see no current-round row and skip forward. Require an existing completed
-- current-round row before any manual advance.
...
BEGIN
  ...
  IF v_game.current_round < 1 THEN
    RETURN QUERY SELECT FALSE, 'Current round has not started'::TEXT;
    RETURN;
  END IF;
  ...
  IF v_current_round_status IS NULL THEN
    RETURN QUERY SELECT FALSE, 'Current round must be created before advancing'::TEXT;
    RETURN;
  END IF;

  IF v_current_round_status <> 'completed' THEN
...
'Advances only from an existing completed current round, preventing duplicate calls from skipping uncreated rounds.';
