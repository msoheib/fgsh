-- Close category prompts as soon as their round exists.
-- This prevents TVs from treating historical prompt rows as active UI state.
...
BEGIN
  ...
  IF FOUND THEN
    UPDATE public.game_category_prompts gcp
    SET
      selected_category = COALESCE(
        gcp.selected_category,
        p_category,
        q.category,
        NULLIF(gcp.options ->> 0, ''),
        'round_created'
      ),
      updated_at = NOW()
    ...
    WHERE gcp.game_id = p_game_id
      ...
      AND gcp.selected_category IS NULL
      AND q.id = v_existing_round.question_id;
...
LEFT JOIN public.questions q
  ON q.id = gr.question_id
WHERE gcp.game_id = gr.game_id
  AND gcp.round_number = gr.round_number
  AND gcp.selected_category IS NULL;
...
'Creates a round and closes the corresponding category prompt when the round exists, preventing TVs from rendering stale category selection state.';
