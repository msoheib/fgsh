...
-- Purpose: Create or update 'deicmaldrown@gmail.com' to be an admin.
...
DECLARE
  v_user_id UUID;
  v_email TEXT := 'deicmaldrown@gmail.com';
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    ...
    VALUES (
      ...
      'email',
      v_user_id::text, -- Often just the user ID for email provider
      now(),
      now(),
      now()
    );
  ...
  -- The trigger 'handle_new_user' might have created the profile, or not.
  INSERT INTO public.host_profiles (id, display_name, is_admin, is_approved)
  VALUES (v_user_id, 'Admin', true, true)
