-- Fix the prior next-round RPC hardening migration for the production schema.
--
-- Production game_rounds rows do not have updated_at. Keep the authoritative
-- duplicate-row selection, but order ties by created_at only.
...
'Advances to the next round using phase-captain authority and authoritative duplicate-round status selection without relying on game_rounds.updated_at.';
